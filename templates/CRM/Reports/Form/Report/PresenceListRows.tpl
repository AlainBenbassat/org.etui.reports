{if (!$chartEnabled || !$chartSupported )&& $rows}
    ...
    {capture assign="tableHeader"}
        {foreach from=$columnHeaders item=header key=field}
            ...
            {if !$skip}
                {if $header.colspan}
                    <th colspan={$header.colspan} style="{$etuiColHeaderStyle}">{$header.title}</th>
                    {assign var=skip value=true}
                    {assign var=skipCount value=`$header.colspan`}
                    {assign var=skipMade  value=1}
                {else}
                    <th {$class} style="{$etuiColHeaderStyle}">{$header.title}</th>
    ...
    {capture assign=sectionHeaderTemplate}
        ...
        {foreach from=$sections item=section key=column name=sections}
            ...
            {/if}
            {$l}/if{$r}
            <tr class="crm-report-sectionHeader crm-report-sectionHeader-{$h}"><th colspan="{$columnCount} style="{$etuiColHeaderStyle}">
    ...
    {foreach from=$rows item=row key=rowid}
        ...
        <tr  class="{cycle values="odd-row,even-row"} {$row.class} crm-report" id="crm-report_{$rowid}">
            {foreach from=$columnHeaders item=header key=field}
                ...
                <td style="{$etuiCellStyle} class="crm-report-{$field}{if $header.type eq 1024 OR $header.type eq 1 OR $header.type eq 512} report-contents-right{elseif $row.$field eq 'Subtotal'} report-label{/if}">
                    {if $row.$fieldLink}
